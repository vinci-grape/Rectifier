        ...
        while max_attempts > 0:
            try:
                response = openai.ChatCompletion.create(
                    model="gpt-3.5-turbo-16k-0613",  # The name of the OpenAI chatbot model to use
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='run translation with GPT-3.5 with a given dataset and languages')
    ...
    with Translate(args.dataset) as translator:
        logging.info(f"translating examples from {source} to {target} using GPT-3.5 and {args.dataset} dataset")
