...
if [[ $MODEL == "ChatGPT" ]]; then
  python3 src/translation/translate_chatgpt.py --dataset $DATASET --source_lang $SRC_LANG --target_lang $TRG_LANG --k $K --p $P --temperature $TEMPERATURE;
elif [[ $MODEL == "StarCoder" || $MODEL == "CodeGen" || $MODEL == "CodeLlama" ]]; then
