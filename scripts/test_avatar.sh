...
if [[ $FILE_NAME ]]; then
  python3 src/test/compile_avatar.py --source_lang $SRC_LANG --target_lang $TRG_LANG --model $MODEL --report_dir $OUTPUT_DIR --file_name $FILE_NAME;
  python3 src/test/compile_avatar_feedback.py --source_lang $SRC_LANG --target_lang $TRG_LANG --model $MODEL --report_dir $OUTPUT_DIR --attempt $ATTEMPT --file_name $FILE_NAME;
else
